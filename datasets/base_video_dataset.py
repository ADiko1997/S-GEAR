...
def convert_to_anticipation(df: pd.DataFrame,
    ...
    logging.info(f'Using future ratios: {future_clip_ratios}')
    logging.info(f'tau_a: {tau_a}')
    ...
    for i, future_clip_ratio in enumerate(future_clip_ratios):
        if future_clip_ratio == -999:
            ...
            logging.info(f'eff tau: {eff_tau_a}')
...
class BaseVideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.similarities_dict = similarities_dict
        self.visual_embeddings_dict = visual_embeddings_dict
        self.tubelets = tubelets
        self.is_train = is_train
        logging.info(f"Usinf dynamic context: {self.is_train}")
        ...
        #create different reader functions based on the __target__ in the config file, useful for multimodal approaches
        targets = reader_fn['_target_'] #List
        if len(targets) == 1:
            self.reader_fn = {}
            reader_fn_ = reader_fn.copy()
            reader_fn_['_target_'] = targets[0]         
            self.reader_fn['mod_0'] = hydra.utils.instantiate(reader_fn_)
        else:
            self.reader_fn = {}
            for idx, target in enumerate(targets):
                if "RULSTM" in target:
                    reader_fn_ = reader_fn.copy()
                    reader_fn_['_target_'] = target
                else:
                    reader_fn_ = {'_target_': target}
                self.reader_fn[f"mod_{idx}"] = hydra.utils.instantiate(reader_fn_)
        ...
        logging.info(f"Returnin future feats: {return_future_clips_too}")
    ...
    def _sample(cls, video_path: Path, fps: float, start: float, end: float,
                ...
                rng: np.random.Generator, is_train: bool):
        """
        ...
        """
        # print(f"cls is train: {is_train}")
        start = max(start, 0)  # No way can read negative time anyway
        #add new startr random for variable video length
        ...
        if is_train:
            new_start = start + torch.rand([1]).item()*0.5*(end-start) #if diff = 0 , new_start= 0
            # print(f"cls is train: {is_train} start: {start} new_start: {new_start} fps: {fps} new_fps: {new_fps}")
            # fps = new_fps
            start = new_start
    ...
    def _get_video(self, df_row):
        ...
        video_paths = {mod: get_abs_path(self.root, df_row['video_path']) for mod in self.reader_fn.keys()}
        fps = {mod: self.reader_fn[mod].get_frame_rate(df_row['video_path']) for mod in self.reader_fn.keys()}
        fps = {}
        for mod in self.reader_fn.keys():
            try:
                fps[mod] = self.reader_fn[mod].get_frame_rate(video_paths[mod])['fps']
            except:
                fps[mod] = self.reader_fn[mod].get_frame_rate(video_paths[mod])
        ...
        for mod in self.reader_fn.keys():
            ...
            frames_subsampled,
            info) = self._sample(video_paths[mod], fps[mod], df_row['start'], df_row['end'],
                                df_row, self.frames_per_clip, self.frame_rate,
                                self.sample_strategy, self.reader_fn[mod], self.rng, self.is_train)
            ...
            if self.return_future_clips_too:
                ...
                for future_id in range(nfutures):
                    video_future, _, _, _, _ = self._sample(
                        ...
                        self.sample_strategy_future, self.reader_fn[mod], self.rng, self.is_train)
    ...
    def _get_vidseg_labels(self, df_row, video_frame_sec: torch.Tensor, tubelets=1):
        """
        Args:
        ...
        if tubelets > 1: #adapt to number of tubelets
            labels['action'] = labels['action'][::tubelets, :]
        return labels
    ...
    #     key, val = list(label_idx.items())[0]
    #     # print(val)
    #     target_language_embeddings = self.embeddings_dict[abs(val)]
    #     target_embeddings_similarities = self.similarities_dict[abs(val)]
    ...
    #         "target_similarities":
    #         target_embeddings_similarities,
    #         'audio': [],  # TODO?
    #         'orig_vid_len':
    ...
    #         video_dict.update({
    #             'target_subclips':
    #             self._get_vidseg_labels(df_row, video_dict['video_frame_sec'], self.tubelets)
    #         })
        
    #     #Append to the dictionary the most frequent label embedding for past frames A.D.
    #         # print(video_dict['target_subclips']['action'])
    #         past_idx = []
    #         for i in video_dict['target_subclips']['action']:
    #             past_idx.append(i.item())

    #         # wts = 0 if torch.where( video_dict['target_subclips']['action']>=0, 1, 0).sum() == 0 else 1
    #         wts = torch.where( video_dict['target_subclips']['action']>=0, 1, 0)
    #         encodings_ = []
    #         similarities_ = []
    #         for i in past_idx:
    #             # if i != -1:
    #             encodings_.append(self.embeddings_dict[torch.abs(torch.tensor(i)).item()])
    #             similarities_.append(self.similarities_dict[torch.abs(torch.tensor(i)).item()])

            
    #         # if len(encodings_) == 0:
    #         #     encodings_.append(self.embeddings_dict[0])
    #         #     similarities_.append(self.similarities_dict[0])

    #         # past_reference_embedding = torch.stack(encodings_).mean(dim=0)
    #         # past_reference_similarities = torch.stack(similarities_).mean(dim=0)

    #         past_reference_embedding = torch.stack(encodings_)
    #         past_reference_similarities = torch.stack(similarities_)

    #         # video_dict.update(
    #         #     {
    #         #         "past_reference_embedding": past_reference_embedding,
    #         #         "past_reference_similarities": past_reference_similarities,
    #         #         "past_wts": wts,
    #         #     }
    #         # )
    ...
    #         most_frequent_label = torch.mode(video_dict['target_subclips']['action'].reshape(1, -1), dim=-1)[0]
    #         video_dict.update(
    #             {
    #                 "past_reference_embedding": self.embeddings_dict[torch.abs(most_frequent_label).item()],
    #                 "past_reference_similarities": self.similarities_dict[torch.abs(most_frequent_label).item()],
    #                 "past_wts": wts,
    #             }
    #         )
    ...
    def __getitem__(self, idx):
        ...
        if df_row is None:
            return None
        output_dict = self._get_video(df_row)
        for mod in output_dict.keys():
            video_dict = output_dict[mod]
            if 'future_0_video' in video_dict:
                video_dict['video'] = torch.cat([video_dict['video'], video_dict['future_0_video'][:,0].unsqueeze(1)],dim=1)
                video = video_dict['video']
    ...
    def _class_balance_data_idx(self, idx):
        ...
        try:
            ...
            if classes_counts['action'][-1] == 0:
                del classes_counts['action'][-1]
        ...
        try:
            ...
            cls_counts = [classes_counts['action'][i] for i in sorted(classes_counts['action'].keys())]
            cls_cumsum = np.cumsum(cls_counts).tolist()
            cls_firstelt = [0] + cls_cumsum[:-1]
            share_per_class = max(cls_counts)
