...
class EPICKitchens(BaseVideoDataset):
    ...
    def __init__(
        ...
        super().__init__(df, self.embeddings_dict, self.similarities_dict,  self.visual_embeddings_dict, tubelets, self.is_train, **other_kwargs)
...
class EpicRULSTMFeatsReader(Reader):
    ...
    def _read_rulstm_features(self,
        ...
        # Sanity check
            logging.debug('Read %s %s %s %s %s %s',
                         lmdb_env, Path(video_path).stem + '_frame_{:010d}.jpg',
                          all_feats[-1].shape, all_feats[-1].min(),
                          all_feats[-1].max(), all_feats[-1].mean())

        final_feat = torch.cat(all_feats, dim=-1)
        # Must return rgb, audio, info; so padding with empty dicts for those
        # Final feats for multimodal models are of shape 1, 1, rgb:flow:...:audio based on the number of different modalities
        return final_feat, {}, {}
