...
def train_one_epoch(
    ...
    optimizer.zero_grad()
    logger.info('Zero initialized optimizer')
    for i, data in enumerate(
        ...
        data, _, losses, accuracies = train_eval_op(data, train_mode=True, epoch=cur_epoch)
        ...
        if (step_id + 1)%accumulate_steps == 0:
            # print(f"Accumulated opt step with id: {step_id+1} and acc_steps: {accumulate_steps}")
            optimizer.step()
            optimizer.zero_grad()
...
def evaluate(
    ...
    for data_key, data_loader in data_loaders.items():
        ...
        for data in metric_logger.log_every(data_loader, 2, header):
            ...
            if store:
                ...
                all_logits.update({'idx': data['mod_0']['idx'].detach().cpu().numpy()})
                uid_data = np.array(data['mod_0']['uid'])
                ...
                if not only_run_featext:
                    ...
                    all_logits.update({
                        ...
                        for key, val in data['mod_0']['target'].items()
                    })
    ...
    if train_eval_op.store_embeddings:
        torch.save(train_eval_op.store_embeddings, store_embeddings_path)
    return main_metric
...
def init_model(model, ckpt_path, modules_to_keep, logger, from_pretrained):
    ...
    if from_pretrained:
        ...
        logging.warning("Loaded from pretraing built-in function")
        return
...
def main(cfg):
    ...
    # Data loading code
    logger.info(dist_info['distributed'])
    ...
    logger.info("Creating data loaders with %d workers", cfg.data_train.workers)
    train_sampler = None
    ...
    if cfg.model.pretrained_detector:
        ckpt_det = torch.load(cfg.model.pretrained_detector_weights, map_location=device)
        state_dict_det = ckpt_det['model']
        pretrained_dict_obj = {"".join(['detector.',k]): v for k, v in state_dict_det.items()}
        
        logger.info(f"Loading this weights from pretrained Obj model: {pretrained_dict_obj.keys()}")
        model.load_state_dict(pretrained_dict_obj, strict=False)
    ...
    if os.path.isfile(last_saved_ckpt):
        ...
        try:
            optimizer.load_state_dict(checkpoint['optimizer']) #For ablation study purposes
        except:
            pass
    ...
    if cfg.train.use_mixup:
        logger.info("Building MixUp function")
        mixup_fn = MixUp(mixup_alpha = cfg.train.mixup_alpha, 
    ...
    for epoch in range(start_epoch, cfg.train.num_epochs):
        ...
        last_saved_time = hydra.utils.call(cfg.train.train_one_epoch_fn,
                                           ...
                                           last_saved_time, accumulate_steps
                                           )

        if cfg.transductive:
            for data_key, data_loader_ in data_loaders_test.items():
                last_saved_time_2 = hydra.utils.call(cfg.train.train_one_epoch_fn,
                                                train_eval_op, optimizer,
                                                lr_scheduler, data_loader_, epoch,
                                                partial_epoch,
                                                stat_loggers["train"], logger,
                                                last_saved_time, accumulate_steps
                                                )
        ...
        if cfg.train.train_one_epoch_fn.save_freq != 0:
            if epoch % cfg.train.train_one_epoch_fn.save_freq == 0:
                store_checkpoint(f'checkpoint_{epoch+1}.pth', model, optimizer, 
                                lr_scheduler, epoch + 1)
